...
export const TranspositionCard = ({
  ...
  const renderAssembleia = () => {
    ...
    // Se for igual à forma de tocar do salmista, não precisa mostrar separado
    ...
    return (
      <div className="instruction-block" onClick={() => setShowSobreModal('assembly')} style={{ cursor: 'pointer' }}>
        <div className="instruction-label" style={{ display: 'flex', alignItems: 'center' }}>
          Assembleia <Info size={14} style={{ marginLeft: '4px', opacity: 0.5 }} />
        </div>
        <div className="badges-row">
          <span className="badge-inst badge-inst-assembly-chord">🎸 Toque {assCapo.formaAcorde}</span>
          {assCapo.capoCasa > 0 && (
    ...
    <div className="transpo-card-wrapper">
      ...
      <div className="transpo-controls-container">
        <div className="transpo-controls">
          <button className="btn-transpo" onClick={() => setTransposition((t) => t - 1)}>-</button>
      ...
      <div className="transpo-instructions">
        {showPlayerInst && (
          <div className="instruction-block" onClick={() => setShowSobreModal('psalmist')} style={{ cursor: 'pointer' }}>
            <div className="instruction-label" style={{ display: 'flex', alignItems: 'center' }}>
              Você toca <Info size={14} style={{ marginLeft: '4px', opacity: 0.5 }} />
            </div>
            <div className="badges-row">
              <span className="badge-inst badge-inst-chord">🎸 Toque {capoInfo.formaAcorde}</span>
              {capoInfo.capoCasa > 0 && (
                <span className="badge-inst badge-inst-capo">
                  <img src={capoIcon} alt="Capo" style={{ width: '14px', height: '14px' }} /> 
      ...
      <div className="transpo-actions">
        ...
        <div className="action-row">
          <button className="btn-action btn-recalibrate" onClick={aplicarTomInteligente}>
            <Settings2 size={16} style={{ marginRight: '0.4rem' }} /> Recalibrar
          </button>
          {user && (
            <button 
            ...
            >
              <ThumbsUp size={16} style={{ marginRight: '0.4rem' }} /> {isToneSaved ? "Favoritado" : "Favoritar"}
            </button>
          )}
        </div>
