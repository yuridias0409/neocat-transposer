...
export function calcularTomIdealInteligente(
  vozSalmista,
  canto,
  perfilUsuario = {},
  cantoFirebaseData = {},
  ...
  if (iaDataVoice) {
    ...
    if (
      maxVotes >= 2 &&
      maxVotes / totalVotes >= 0.5 &&
      bestTransposition !== null
    ) {
      return {
  ...
  const offsetFromSingerLowest =
    songWideness >= singerWideness
      ? 0
  ...
  const singerRangeHz = {
    min: vozSalmista.minHz,
    max: vozSalmista.maxHz,
  };
  const optResult = HzKeyOptimizer.findOptimalKeyHz(
    songFrequenciesHz,
    singerRangeHz,
    "MIXED",
    1.5,
  ...
  const offsetPessoal =
    perfilUsuario?.preferencias_tom?.offset_pessoal_semitones || 0;
  ...
  if (!offsetGlobalCanto && metricasCanto.total_avaliacoes > 5) {
    const propAlto =
      (metricasCanto.qtd_alto_demais || 0) / metricasCanto.total_avaliacoes;
    const propBaixo =
      (metricasCanto.qtd_baixo_demais || 0) / metricasCanto.total_avaliacoes;
  ...
  const folgaAguda = Math.floor(
    salmistaMaxSemi - (cantoMaxSemi + semitonesFinal),
  );
  ...
  if (offsetGlobalCanto)
    mensagem += " Ajuste comunitário de " + offsetGlobalCanto + ".";
  return {
