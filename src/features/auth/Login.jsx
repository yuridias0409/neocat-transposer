...
const Login = ({ onLogin }) => {
    ...
    if (email.trim() !== "" && password.trim() !== "") {
      ...
      try {
        ...
        if (
          ...
          setErrorMsg(
            "Senha incorreta. Se você nunca definiu uma senha, use o acesso sem senha.",
          );
        } else if (error.code === "auth/email-already-in-use") {
          setErrorMsg(
            'Este e-mail já possui uma conta (provavelmente você logava via link). Use o "Acesso sem senha" uma vez para entrar e, se desejar, defina uma senha depois.',
          );
        } else if (error.code === "auth/weak-password") {
          setErrorMsg("A senha deve ter pelo menos 6 caracteres.");
        } else if (error.code === "auth/operation-not-allowed") {
          setErrorMsg(
            "O login com senha está desativado no painel do Firebase. Ative-o em Authentication -> Sign-in method.",
          );
        } else {
  ...
  const handleMagicLink = async () => {
    ...
    if (!email.trim()) {
      setErrorMsg(
        "Por favor, digite seu e-mail no campo acima para receber o link.",
      );
      return;
    }
    setLoading(true);
  ...
  const handlePasswordReset = async () => {
    ...
    if (!email.trim()) {
      setErrorMsg(
        "Por favor, digite seu e-mail para enviarmos o link de criação de senha.",
      );
      return;
    }
    setLoading(true);
      ...
      <div className="login-card card text-center">
        ...
        {magicLinkSent ? (
          ...
          >
            ...
            >
              ✅ Um link foi enviado para <strong>{email}</strong>. Verifique
              seu e-mail e clique nele para entrar ou criar sua senha!
            </p>
            <button
          ...
          <>
            <p className="mb-4">
              Entre com sua conta ou criaremos uma automaticamente para você.
            </p>
