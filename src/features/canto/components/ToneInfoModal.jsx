...
const ComfortMapVisualization = ({
  ...
  const getPercent = (freq) => {
    ...
    return Math.max(
      0,
      Math.min(100, ((p - minPitch) / (maxPitch - minPitch)) * 100),
    );
  };
...
export const ToneInfoModal = ({
  ...
  const activeTransposition =
    isAssembly && tomEsforco !== null && tomEsforco !== undefined
      ? tomEsforco
      : transposition;
  const activeCapoInfo = isAssembly
    ? otimizarCapoETom(canto.tom_original, activeTransposition)
    : capoInfo;
  const titleText = isAssembly
    ? "Análise do tom da Assembleia"
    : "Análise do seu tom";
  ...
  if (!transposedMinFreq || !isFinite(transposedMinFreq))
    transposedMinFreq = 110;
  if (!transposedMaxFreq || !isFinite(transposedMaxFreq))
    transposedMaxFreq = 330;
  return (
    <div
      style={{
        position: "fixed",
    ...
    >
      <div
        ...
        style={{
          width: "100%",
          maxWidth: isAssembly ? "500px" : "900px",
          position: "relative",
          padding: "1.5rem",
          display: "flex",
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div
                className={`badge-inst ${isAssembly ? "badge-inst-assembly-chord" : "badge-inst-chord"}`}
              >
                🎸 Toque {activeCapoInfo.formaAcorde}
              </div>
              {activeCapoInfo.capoCasa > 0 && (
                <div
                  className={`badge-inst ${isAssembly ? "badge-inst-assembly-capo" : "badge-inst-capo"}`}
                >
                  <img
                    src={capoIcon}
                    alt="capo"
          ...
          {isAssembly ? (
            <>
              <AssemblyStatus
                canto={canto}
                transposition={activeTransposition}
              />
              <ComfortMapVisualization
                title="Mapa de Calor Vocal"
                songMin={transposedMinFreq}
                songMax={transposedMaxFreq}
                targetMin={110}
                targetMax={246.94}
                alignOctaves={false}
            ...
            <div className="tone-modal-grid">
              <div className="tone-modal-col">
                ...
                <ComfortMapVisualization
                  ...
                  songMax={transposedMaxFreq}
                  targetMin={
                    userProfile?.min?.freq || userProfile?.f0_min || 110
                  }
                  targetMax={
                    userProfile?.max?.freq || userProfile?.f0_max || 330
                  }
                  alignOctaves={true}
                  labelTarget="Sua Voz"
                />
