...
export function otimizarCapoETom(tomOriginal, semitonesDesejados) {
  ...
  let casaCapo = 0;
  const formasAmigaveis = isMenor
    ? FORMAS_AMIGAVEIS_MENORES
    : FORMAS_AMIGAVEIS_MAIORES;
  if (semitonesDesejados >= 0 && semitonesDesejados <= 4) {
    const notaOriginal = ESCALA[idxOriginal];
    melhorForma = notaOriginal + (isMenor ? "-" : "");
    casaCapo = semitonesDesejados;
  } else {
