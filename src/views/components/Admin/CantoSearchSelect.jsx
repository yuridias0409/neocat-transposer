...
const colorMap = {
  verde: '#dcfce7',    // verde pastel
  amarelo: '#fef9c3',  // amarelo pastel
  azul: '#e0f2fe',     // azul pastel
  branco: '#ffffff'
};
...
export function CantoSearchSelect({ value, onChange }) {
  ...
  return (
    <div ref={wrapperRef} style={{ position: 'relative', width: '100%', fontFamily: 'var(--font-body)' }}>
      <div 
      ...
      >
        <span style={{ color: selectedCanto ? '#333' : '#666', fontWeight: selectedCanto ? 'bold' : 'normal', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {selectedCanto ? selectedCanto.titulo : '-- Buscar Canto --'}
        </span>
        <ChevronDown size={18} color="#666" style={{ transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: '0.2s' }} />
      </div>
        ...
        <div style={{
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {filteredCantos.length === 0 ? (
              <div style={{ padding: '12px', color: '#999', textAlign: 'center', fontSize: '0.9rem' }}>Nenhum canto encontrado.</div>
            ) : (
