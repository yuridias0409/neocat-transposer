...
export async function processarFeedbackEAprender({
  userId,
  cantoId,
  tipoFeedback,
  tomAtualSemitons,
  ...
  if (tipoFeedback !== "OPTIMAL") {
    ...
    if (cantoData && cantoData.freq_max_curada && cantoData.freq_min_curada) {
      const freqMaxReal =
        cantoData.freq_max_curada * Math.pow(2, tomAtualSemitons / 12);
      const freqMinReal =
        cantoData.freq_min_curada * Math.pow(2, tomAtualSemitons / 12);
    ...
    if (cantoData && cantoData.freq_max_curada && cantoData.freq_min_curada) {
      ...
      if (maxNote && maxNote !== "--")
        comfortMap[maxNote] = (comfortMap[maxNote] || 0) + 1;
      if (minNote && minNote !== "--")
        comfortMap[minNote] = (comfortMap[minNote] || 0) + 1;
    }
    if (profile.tipoVoz) {
  ...
  try {
    const localProfile =
      JSON.parse(localStorage.getItem("userVoiceProfile")) || {};
  ...
  if (cantoData) {
    let metricas = cantoData.metricas_feedback || {
      qtd_otimo: 0,
      qtd_alto_demais: 0,
      qtd_baixo_demais: 0,
      total_avaliacoes: 0,
    ...
    if (metricas.qtd_alto_demais >= 20) {
      if (cantoData.freq_max_curada)
        updates.freq_max_curada = parseFloat(
          (cantoData.freq_max_curada * Math.pow(2, -1 / 12)).toFixed(2),
        );
      if (cantoData.freq_min_curada)
        updates.freq_min_curada = parseFloat(
          (cantoData.freq_min_curada * Math.pow(2, -1 / 12)).toFixed(2),
        );
      if (cantoData.freq_max_povo_curada)
        updates.freq_max_povo_curada = parseFloat(
          (cantoData.freq_max_povo_curada * Math.pow(2, -1 / 12)).toFixed(2),
        );
      if (cantoData.freq_min_povo_curada)
        updates.freq_min_povo_curada = parseFloat(
          (cantoData.freq_min_povo_curada * Math.pow(2, -1 / 12)).toFixed(2),
        );
