...
export function useCantoController(cantoId, user) {
  ...
  const [transposition, setTransposition] = useState(
    precomputedOffset !== undefined ? precomputedOffset : 0,
  );
  ...
  const [initialTransposition, setInitialTransposition] = useState(
    precomputedOffset !== undefined ? precomputedOffset : null,
  );
  ...
  useEffect(() => {
    ...
    const initTone = async () => {
      ...
      if (
        profileData &&
        profileData.cantos_validados &&
        profileData.cantos_validados[cantoId] !== undefined
      ) {
  ...
  const salvarTomPreferido = async () => {
    ...
    try {
      ...
      if (initialTransposition !== null) {
        ...
        else if (transposition < initialTransposition)
          tipoFeedback = "TOO_HIGH";
      }
      ...
      let up = await UserDAO.getProfile(
      ...
      showToast(
        "✅ Tom favoritado com sucesso! O aplicativo lembrará dessa preferência.",
      );
    } catch (err) {
  ...
  const aplicarTomInteligente = async () => {
    ...
    const cantoData = await CantoDAO.getPitchMetadata(cantoId);
    const resultado = calcularTomIdealInteligente(
      vozSalmista,
      canto,
      userProfile,
      cantoData,
  ...
  return {
    ...
    savedTransposition,
    isToneSaved:
      savedTransposition !== null && savedTransposition === transposition,
    salvarTomPreferido,
  };
}
