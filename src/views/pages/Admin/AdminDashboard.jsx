...
export function AdminDashboard() {
  ...
  const updatePitch = () => {
    ...
    const [pitch, clarity] = detector.findPitch(inputBuffer, audioCtxRef.current.sampleRate);
  ...
  const handleSaveToFirestore = async () => {
    if (!cantoId || hzHistory.length === 0) {
      alert('Faltam dados: Preencha o ID e faça uma gravação/varredura primeiro.');
      return;
    }

    const sorted = [...hzHistory].sort((a, b) => a - b);
    const minCurada = sorted[Math.floor(sorted.length * 0.05)] || sorted[0];
    const maxCurada = sorted[Math.floor(sorted.length * 0.95)] || sorted[sorted.length - 1];
    ...
    const dataToSave = {
      ...
      tom_gravacao: tomGravacao || cantosData[cantoId]?.tom_original || 'Desconhecido',
