  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (currentMicHz > 0) {
        ...
        if (
          hist.length === 0 ||
          Math.abs(hist[hist.length - 1].t - currentTime) > 0.05
        ) {
      ...
      while (
        userPitchHistoryRef.current.length > 0 &&
        userPitchHistoryRef.current[0].t < timeStart
      ) {
      ...
      if (currentRefHz > 0 && currentMicHz > 0) {
        ...
        if (diff > 2.5) {
          setTooltip({
            text: "Cante mais GRAVE! ⬇️",
            type: "warning",
          });
        } else if (diff < -2.5) {
          setTooltip({
            text: "Cante mais AGUDO! ⬆️",
            type: "warning",
          });
        } else {
          if (futureRefHz > 0) {
            ...
            if (futSemi - refSemi > 2.5) {
              setTooltip({
                text: "Atenção: Aqui SOBE ↗️",
                type: "info",
              });
            } else if (futSemi - refSemi < -2.5) {
              setTooltip({
                text: "Atenção: Aqui DESCE ↘️",
                type: "info",
              });
            } else {
              setTooltip({
  ...
  if (!userProfile)
    return (
      ...
      >
        ...
        >
          Calibre sua voz primeiro para usar o Karaoke.
        </p>
      </div>
    );
  return (
    <div
      className="karaoke-panel card mb-3"
      style={{
        padding: "1rem",
    ...
    >
      ...
      >
        🎧 Aviso: Este modo funciona muito melhor com Fones de Ouvido! (Evita
