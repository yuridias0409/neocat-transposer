...
export function AudioPlayerView({
      ...
      if (playerRef?.current && playerRef.current.state === "started") {
        ...
        if (elapsed >= dur) {
          ...
          if (timeTextRef.current)
            timeTextRef.current.innerText = `00:00 / ${formatTime(dur)}`;
          return;
        }
        const perc = (elapsed / dur) * 100;
        if (progressRef.current) progressRef.current.style.width = `${perc}%`;
        if (timeTextRef.current)
          timeTextRef.current.innerText = `${formatTime(elapsed)} / ${formatTime(dur)}`;
      }
  ...
  const onSeek = (e) => {
    ...
    if (timeTextRef.current)
      timeTextRef.current.innerText = `${formatTime(percent * duration)} / ${formatTime(duration)}`;
  };
